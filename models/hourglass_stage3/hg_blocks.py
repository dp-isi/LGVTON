...
def create_front_module(input, num_channels, bottleneck,name_code='default'):
    ...
    _x = bottleneck(_x, int(num_channels // 2), 'front_residual_x1_%s'%(name_code))
    # _x = MaxPool2D(pool_size=(2, 2), strides=(2, 2))(_x)

    _x = bottleneck(_x, int(num_channels // 2), 'front_residual_x2_%s'%(name_code))
...
def create_heads(prelayerfeatures, rf1, num_classes, hgid, num_channels,list_inputs):
    ...
    output_img = Conv2D(3, kernel_size=(1, 1), activation='tanh', padding='same',
